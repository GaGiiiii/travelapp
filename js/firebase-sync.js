...
// Replace firebaseConfig with your own project's.
...
(function () {
    ...
    function status() {
        return {
            ...
            // distinguish "SDK/network failed to load" from "config missing"
    ...
    window.__cloud = {
        ...
        async createCode() {
            ...
            await db.collection(COLLECTION).doc(code).set({
                lists, updatedAt: firebase.firestore.FieldValue.serverTimestamp()
            });
            subscribe(code);
            return code;
        },
